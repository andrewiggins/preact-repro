...
options.unmount = function (vnode) {
  ...
  if (hooks) {
    hooks._list.forEach(function (hook) { return hook._cleanup && hook._cleanup(); });
  }
};
/**
...
function getHookState(index) {
  if (options.hook) { options.hook(currentComponent); } // Largely inspired by:
...
function useReducer(reducer, initialState, init) {
  ...
  if (!hookState._component) {
    ...
    hookState._value = [!init ? invokeOrReturn(null, initialState) : init(initialState), function (action) {
...
function useEffect(callback, args) {
  ...
  var state = getHookState(currentIndex++);
...
function useContext(context) {
  ...
  var state = getHookState(currentIndex++); // This is probably not safe to convert to "!"
...
export { useState, useReducer, useEffect, useLayoutEffect, useRef, useImperativeHandle, useMemo, useCallback, useContext, useDebugValue };
